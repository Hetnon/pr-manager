...
export function makeFsApiFs(root: FileSystemDirectoryHandle): FsApiFs {
    ...
    async function getFile(path: string, create = false): Promise<FileSystemFileHandle> {
        ...
        if (fileName === undefined) throw new ENoEnt(path);
    ...
    async function pathStat(path: string): Promise<StatsLike> {
        ...
        if (name === undefined) throw new ENoEnt(path);
        ...
        promises: {
            ...
            async rmdir(path: string): Promise<void> {
                try {
                    const parts = splitPath(path);
                    const name = parts.at(-1);
                    if (name === undefined) throw new EPerm('cannot rmdir root');
...
function wrapFsError(caughtError: unknown, operation: string, path: string): Error {
    ...
    return new Error(`fsApi ${operation} ${path}: ${name || 'Error'}: ${errorInfo.message ?? stringifyUnknown(caughtError)}`);
}
...
function stringifyUnknown(value: unknown): string {
    ...
    // value is number | bigint | boolean | symbol | function — all have safe String() output
    return (value as number | bigint | boolean | symbol | (() => unknown)).toString();
}
